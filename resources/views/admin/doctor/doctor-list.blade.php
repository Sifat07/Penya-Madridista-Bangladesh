                ...
                <div class="card card-primary">
                    <div class="card-header">
                        ...
                        <div class="card-tools">
                            <a class="btn btn-tool" href="{{ url('admin/add-doctor') }}">Add New</a>
                        </div>
                    </div>
                    <div class="card-body">
                            ...
                            <tbody>
                                @foreach ($data as $key => $row)
                                    <tr>
                                        ...
                                        <td>{{ $row->email }}</td>
                                        <td>@php
                                            $specialities = explode(",", $row->speciality);
                                            foreach($specialities as $speciality){
                                                echo '<span class="badge bg-info" style="font-size: 1rem">'.$speciality.'</span>&nbsp;';
                                            }
                                            @endphp</td>
                                        <td>{!! $row->status == 1 ? '<span class="badge bg-success">active</span>' : '<span class="badge bg-warning">Inactive</span>' !!}</td>
                                        <td>
                                            <form role="form" action="{{ url('/admin/delete-doctor', $row->id) }}" method="post">
                                                @csrf
...
<script>
    ...
    $('.edit-item').click(function(){
        window.location.href = "{{ url('admin/edit-doctor') }}"+"/"+$(this).attr('elem_id');
    });
</script>
